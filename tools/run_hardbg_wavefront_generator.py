...
def _conv_gray(gray: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    weight = kernel.to(gray.device, dtype=gray.dtype).view(1, 1, 3, 3)
    return F.conv2d(gray.unsqueeze(0), weight, padding=1).squeeze(0)


def wavefront_maps(rgb: torch.Tensor) -> torch.Tensor:
    gray = (0.299 * rgb[0:1] + 0.587 * rgb[1:2] + 0.114 * rgb[2:3]).clamp(0, 1)
    kernels = [
    ...
    responses = [torch.abs(_conv_gray(gray, k)) for k in kernels]
    stack = torch.cat(responses, dim=0)
    max_resp = stack.max(dim=0, keepdim=True).values
    mean_resp = stack.mean(dim=0, keepdim=True)
    # Local contrast catches weak object boundaries that are not strong global edges.
    local_mean = F.avg_pool2d(gray.unsqueeze(0), kernel_size=9, stride=1, padding=4).squeeze(0)
    local_contrast = (gray - local_mean).abs()
    wf = torch.cat([max_resp, mean_resp, local_contrast, stack], dim=0)
    denom = wf.flatten(1).amax(dim=1).view(-1, 1, 1).clamp_min(1e-6)
    return (wf / denom).clamp(0, 1)
...
def build_input(
    ...
    illum, ret = retinex_maps(rgb)
    wave = wavefront_maps(rgb)
    return torch.cat([rgb, raw, illum, ret, score, hard_score, reject, hard_reject, wave], dim=0)
...
class HardBackgroundContrastiveGenerator(nn.Module):
    def __init__(self, width: int = 24):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(17, width, 3, padding=1),
...
def objective(logits, gt, x, args):
    ...
    wave = x[:, 10:11]
    # Penalize confident background false positives more strongly when no boundary cue supports them.
    wave_unsupported_bg = (prob * hard_score * (1.0 - wave) * (1.0 - gt)).mean()
    return (
        bce
        + dice
        ...
        + args.wave_bg_weight * wave_unsupported_bg
...
def main():
    ...
    ap.add_argument("--model-name", default="HardBGWavefrontGenerator")
    ...
    ap.add_argument("--wave-bg-weight", type=float, default=0.08)
